...
def calc_xc(density, xgrid, xcfunc, xctype):
    ...
    # special case in which xc = -hartree
        ...
        if xcfunc._family == 2:
            ...
            if config.spindims == 2:
                sigma_libxc = arrays.zeros32((config.grid_params["ngrid"], 3))
                ...
                sigma_libxc = arrays.zeros32((config.grid_params["ngrid"], 1))
            ...
            # extract xc potential
                ...
                if config.spindims == 2:
                    ...
                    xc_arr = arrays.zeros32((config.grid_params["ngrid"], config.spindims))
...
def gga_pot_chainrule(libxc_output, grad_0, grad_1, xgrid, spindims):
    ...
    if spindims == 2:
        ...
        gga_addition2 = 2.0 * arrays.array32(
            (
                np.exp(-2.0 * xgrid)
